...
void DrawTable() {
    ...
    for (int i = 0; i < SP_NUMBER; i++) {
        ...
        FillCircle(screen, currX, currY, PHILOSOPHER_RADIUS, 0x00ff00); 
        ...
        FillRect(screen, stickCenterX - STICK_LENGTH / 2, stickCenterY - STICK_HEIGHT / 2, STICK_LENGTH, STICK_HEIGHT, 0xff0000);
    }
...
void Philosophers::pickSticks() { if (sticks.isAvailable(id) && sticks.isAvailable((id + 1) % SP_NUMBER)) { sticks.take(id); sticks.take((id + 1) % SP_NUMBER); state = Eating; } else state = Hungry; }
void Philosophers::putSticks() { sticks.release(id); sticks.release((id + 1) % SP_NUMBER); }

void ICGUI_Create() {ICG_MWTitle("Dining Philosophers"); ICG_MWSize(790, 610);}
void Table(){CreateImage(screen, 560, 620, ICB_UINT);}

void Start(void* simFunction) {CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)simFunction, NULL, 0, NULL); CreateImage(screen, 740, 520, ICB_UINT);}
